    ...
    function onSuccess(response, body) {
        ...
        if (body.collection.length > 0){
            ...
            promises = _.map(invoices, function (invoice) {
                ...
                //if (!cfg.toDownloadPDF) { DON'T DOWNLOAD ATTACHMENTS FOR A WHILE
                ...
                if (!_.isString(invoice.pdf)) {
                    self.emit('error', new Error('`pdf` field is not a string'));
                    return Q.resolve();
                }

                var pdfURL = invoice.pdf.split('?')[0];
                ...
                request.get(pdfURL, reqOptions)
                    .on('response', function(res){
                        ...
                        attachments.addS3StreamAttachment(newMsg, fileName, res, contLength)
                            .done(function(){
                                self.emit('data', newMsg );
                                deferred.resolve();
                            });
